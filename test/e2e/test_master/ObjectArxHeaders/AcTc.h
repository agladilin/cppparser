...
public:
  ...
  BOOL Render(HDC hDC, int xPos, int yPos, int nWidth, int nHeight, UINT nStyle = ACTC_IMAGE_NORMAL, BOOL bTransparentBitmap = TRUE);
...
public:
  ...
  BOOL GetImageInfo(INT_PTR iIndex, ACTC_IMAGE_INFO* pImageInfo, BOOL bUseDarkTheme = FALSE) const;
  BOOL GetEditImageInfo(INT_PTR iIndex, ACTC_IMAGE_INFO* pImageInfo, BOOL bUseDarkTheme = FALSE);
  BOOL SetImageInfo(INT_PTR iIndex, const ACTC_IMAGE_INFO* pImageInfo, BOOL bUseDarkTheme = FALSE);
...
public:
  ...
  AcTcCatalogItem* GetNextChild(int iIndex, CatalogItemType nType) const;
...
{
public:
  virtual ~AcTcCatalogItemReactor();
...
ACTC_PORT BOOL AcTcDownloadItem(LPCTSTR pszUrl, AcTcCatalogItem*& pNewItem, DWORD dwItemTypes = -1, DWORD dwDownloadFlag = kDownloadAll, LPCTSTR pszDownloadPath = NULL);
ACTC_PORT int AcTcDownloadItem(LPTSTR* pUrls, int nNumUrls, AcTcCatalogItem** pItems, DWORD dwItemTypes = -1, DWORD dwDownloadFlag = kDownloadAll, LPCTSTR pszDownloadPath = NULL);
ACTC_PORT BOOL AcTcRefreshItem(AcTcCatalogItem** pItems, int nNumItems, DWORD dwItemTypes = -1, DWORD dwRefreshFlag = kRefreshAll);
